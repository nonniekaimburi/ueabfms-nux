  ...
  <div class="flex justify-between">
    <div>
      <p class="font-bold text-2xl">
        <span class="text-primary">Admin</span> view
      </p>
    </div>
    <button
      @click="handleUsers"
      class="px-4 right-0 bg-blue-600 py-2 rounded text-white"
    >
            ...
            <table class="min-w-full">
              ...
              <tbody>
                <tr class="border" v-for="(user, index) in users" :key="index">
                  <td
                    v-if="user.admin"
                    class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 border-r-2"
                  >
                    {{ index + 1 }}
                  </td>
                  <td
                    v-if="user.admin"
                    class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap border-r-2"
                  >
                    {{ user.email }}
                  </td>
                  <td
                    v-if="user.admin"
                    class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap first-letter border-r-2"
                  >
                    {{ user.admin }}
                  </td>
                  <td
                    v-if="user.admin"
                    class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap border-r-2 w-36"
                  >
                    <button
                      v-if="!user.isBanned"
                      
                      class="px-4 bg-orange-500 rounded py-2 mx-2"
                    ...
                    <button
                      v-if="user.isBanned"
                      class="px-4 bg-orange-500 rounded py-2 mx-2"
                      @click="handleUnRestrictUser(user.id)"
                    >
                      <span>UnRestrict</span>
                    </button>
                    <nuxt-link
                      :to="{ name: 'user-id', params: { id: user.id } }"
                      class="px-4 bg-green-500 rounded py-2 mx-2"
                    >
                            ...
                            >
                              ...
                              <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                  Do you want to restrict {{ user.email }} from
